...
echo $PATH
cd $localdir
/afs/cern.ch/user/y/yumiao/public/HGCAL_Raw_Data_Handling/CommonModeTB_submission_version/PFCal/PFCalEE/userlib/bin/digitizer -c /afs/cern.ch/user/y/yumiao/public/HGCAL_Raw_Data_Handling/CommonModeTB_submission_version/PFCal/PFCalEE/userlib/DigiConfig.cfg -n 0 -i /eos/cms/store/cmst3/group/hgcal/Geant4//git2022Sep10_c2c1571/pi-/HGcal_version120_model0_BOFF_en${ENERGY}_eta${ETA}_run${Step}.root -o $localdir/ --granulStr=0-1:1  --noiseStr=0-1:0.15 --threshStr=0-1:5 --interCalib=${IC} --nSiLayers=3 --nPU=${NPUVTX} --puPath=root://eoscms//eos/cms/store/cmst3/group/hgcal/Standalone/V12/MinBias/ -a false
echo "--Local directory is " $localdir
echo home=$HOME
echo path=$PATH
echo ldlibpath=$LD_LIBRARY_PATH
ls *
eos mkdir -p /eos/cms/store/cmst3/group/hgcal/Geant4//git2022Sep10_c2c1571/pi-
eos cp $localdir/DigiPFcal.root /eos/cms/store/cmst3/group/hgcal/Geant4//git2022Sep10_c2c1571/pi-/Digi_300uversion120_model0_BOFF_npuvtx${NPUVTX}_ic${IC}_en${ENERGY}_eta${ETA}_run${Step}.root
if (( "$?" != "0" )); then
echo " --- Problem with copy of file DigiPFcal.root to EOS. Keeping locally."
else
eossize=`eos ls -l /eos/cms/store/cmst3/group/hgcal/Geant4//git2022Sep10_c2c1571/pi-/Digi_300uversion120_model0_BOFF_npuvtx${NPUVTX}_ic${IC}_en${ENERGY}_eta${ETA}_run${Step}.root | awk '{print $5}'`
...
echo " --- File DigiPFcal.root successfully copied to EOS: /eos/cms/store/cmst3/group/hgcal/Geant4//git2022Sep10_c2c1571/pi-/Digi_300uversion120_model0_BOFF_npuvtx${NPUVTX}_ic${IC}_en${ENERGY}_eta${ETA}_run${Step}.root"
rm DigiPFcal.root
fi
fi
