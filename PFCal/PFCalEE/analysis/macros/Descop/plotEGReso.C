...
int plotEGReso(){//main
  ...
  for (unsigned ic(0);ic<nRemove;++ic){//loop on intercalib
	...
	for (unsigned ipu(0); ipu<nPu; ++ipu){//loop on pu
	  ...
	  for (unsigned iL(0);iL<nLayers;++iL){//loop on layers
		...
		for (unsigned jL(0);jL<30;++jL){
		  ...
		  if (!firstLayer)lName << "+";
		  lName << "(" << absSum.str() << "absWeight(" << jL << ")"  << ")*energy_" << jL << "_SR" << iSR ;
		  absSum.str("");
		  firstLayer=false;
		}
	      }
	...
	for (unsigned iSR(0); iSR<nSR;++iSR){//loop on signal region
	  ...
	  grDummy->Draw("AP");
	  for (unsigned iL(0);iL<nLayers;++iL){//loop on lay
